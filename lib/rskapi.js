...
function Host(provider) {
	...
	this.getStorageAt = function (address, offset, cb) {
		...
		provider.call('eth_getStorageAt', [utils.toHexAddress(address), utils.toHex(offset), 'latest'], function (err, data) {
			if (err)
				cb(err, null);
			else
				cb(null, data);
	...
	this.getTransactionCount = function (address, block, cb) {
		...
		provider.call('eth_getTransactionCount', [utils.toHexAddress(address), block], cb);
	};
...
function Client(options) {
    ...
    this.nonce = function (address, cb) {
        ...
        host.getTransactionCount(address, cb);
    }
