...
function Client(options) {
    ...
    this.balance = function (address, cb) {
        ...
        host.getBalance(toAddress(address), cb);
    }
    ...
    this.receipt = function (hash, times, cb) {
        ...
        host.getTransactionReceiptByHash(hash, getReceipt);
    ...
    this.invoke = function (from, to, fn, args, options, cb) {
        ...
        function sendTransaction() {
            const tx = {
                ...
                data: to ? '0x' + simpleabi.encodeCall(fn, args) : toHex(fn) + simpleabi.encodeValues(args)
            };
